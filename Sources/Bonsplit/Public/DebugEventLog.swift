...
public final class DebugEventLog: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "atelier.debug-event-log")
    ...
    private static func resolveLogPath() -> String {
        ...
        if let explicit = env["ATELIER_DEBUG_LOG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !explicit.isEmpty {
            return explicit
        }

        if let tag = env["ATELIER_TAG"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !tag.isEmpty {
            return "/tmp/atelier-debug-\(sanitizePathToken(tag)).log"
        }

        if let socketPath = env["ATELIER_SOCKET_PATH"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !socketPath.isEmpty {
            let socketBase = URL(fileURLWithPath: socketPath).deletingPathExtension().lastPathComponent
            if socketBase.hasPrefix("atelier-debug-") {
                return "/tmp/\(socketBase).log"
            }
        }

        if let bundleId = Bundle.main.bundleIdentifier,
           bundleId != "com.atelierterm.app.debug" {
            return "/tmp/atelier-debug-\(sanitizePathToken(bundleId)).log"
        }

        return "/tmp/atelier-debug.log"
    }
